...
fn fsevents_repair_schedule_retries_transient_failure_from_binary() {
    ...
    assert!(
        catalog_text.contains(&format!(
            "payload\t1\tindexing\tfsevents repair schedule\t{}\t",
            cursor.display()
        )) && catalog_text.contains("\tvisible:fsevents repair schedule:adaptive"),
        "{catalog_text}"
    );
    let progress_text = fs::read_to_string(&progress).unwrap();
    assert!(
        progress_text.contains("progress\t1\tvisible\tvisible\tfsevents repair schedule\t")
            && progress_text.contains("\tcompleted\t3\t3\tcompleted:1 jobs:0 dropped_roots"),
        "{progress_text}"
    );
...
fn searches_persisted_tags_from_binary() {
    ...
    assert!(
        sidecar_search_stderr.contains(&format!(
            ...
            content.display()
        )) && sidecar_search_stderr.contains("admitted-volume-count=1\t")
            && sidecar_search_stderr.contains("\tadmitted-volume-ids=")
            && !sidecar_search_stderr.contains("\tadmitted-volume-ids=-\t")
    ...
    assert!(
        sidecar_budget_stderr.contains(&format!(
            ...
            && sidecar_budget_stderr.contains("admitted-volume-count=1\t")
            && sidecar_budget_stderr.contains("\tadmitted-volume-ids=")
            && !sidecar_budget_stderr.contains("\tadmitted-volume-ids=-\t")
    ...
    assert!(
        sidecar_session_stderr.contains(&format!(
            ...
            && sidecar_session_stderr.contains("admitted-volume-count=1\t")
            && sidecar_session_stderr.contains("\tadmitted-volume-ids=")
            && !sidecar_session_stderr.contains("\tadmitted-volume-ids=-\t")
    ...
    let session_retry_catalog_text = fs::read_to_string(&session_retry_catalog).unwrap();
    assert!(
        session_retry_catalog_text.contains(&format!(
            "payload\t1\tindexing\tsidecar session\t{}\t",
            index.display()
        )) && session_retry_catalog_text.contains("\tvisible:sidecar session:adaptive"),
        "{session_retry_catalog_text}"
    );
    let session_retry_progress_text = fs::read_to_string(&session_retry_progress).unwrap();
    assert!(
        session_retry_progress_text.contains("progress\t1\tvisible\tvisible\tsidecar session\t")
            && session_retry_progress_text.contains("\tcompleted\t3\t3\tcompleted\t"),
        "{session_retry_progress_text}"
    );
    ...
    assert!(
        sidecar_scoped_stderr.contains(&format!(
            ...
            && sidecar_scoped_stderr.contains("admitted-volume-count=1\t")
            && sidecar_scoped_stderr.contains("\tadmitted-volume-ids=")
            && !sidecar_scoped_stderr.contains("\tadmitted-volume-ids=-\t")
    ...
    assert!(
        sidecar_empty_scope_stderr.contains("sidecar-volume-scope")
            && sidecar_empty_scope_stderr.contains("admitted-volume-count=0\t")
...
fn searches_persisted_text_content_from_binary() {
    ...
    assert!(
        stderr.contains(&format!(
            ...
            && stderr.contains("\tstable-id=")
            && !stderr.contains("\tstable-id=-\t")
            && stderr.contains("\twritable=false\t")
            && stderr.contains("\tread-only=true\t")
            && stderr.contains("\treachable=true\t")
            && stderr.contains("\treason=cached-volume-report")
...
fn index_content_retries_transient_failure_from_binary() {
    ...
    assert!(
        catalog_text.contains(&format!(
            "payload\t1\tindexing\tcontent index\t{}\t",
            content.display()
        )) && catalog_text.contains("\tvisible:content index:adaptive"),
        "{catalog_text}"
    );
    let progress_text = fs::read_to_string(&progress).unwrap();
    assert!(
        progress_text.contains("progress\t1\tvisible\tvisible\tcontent index\t")
            && progress_text.contains("\tcompleted\t3\t3\tcompleted:2 records:1 indexed\t"),
        "{progress_text}"
    );
...
fn native_app_launch_applies_capture_window_placement_from_binary() {
    ...
    assert!(stdout.contains("\nsession-visible\tplacement=40,70,800,500\tcascade=0\tactivation=activate-app-and-focus-new-window\tfocus=true\tshow=true\ttabs=gfm-main-window\taffordances=movable-enabled,resizable-enabled,minimizable-enabled"), "{stdout}");
    assert!(stdout.contains("icon-view\t"), "{stdout}");
    assert!(stdout.contains("Visible.txt"), "{stdout}");
