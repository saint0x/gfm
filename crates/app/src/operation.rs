...
fn operation_volume_copy_policy_report(operation: &Operation) -> Result<String> {
    ...
    Ok(match operation {
        Operation::Copy { from, to } | Operation::Move { from, to } => {
            ...
            let source_writable = operation_volume_writable_for_path(&report, from);
            let destination_writable = operation_volume_writable_for_path(&report, to);
            let source_read_only = operation_volume_read_only_for_path(&report, from);
            let destination_read_only = operation_volume_read_only_for_path(&report, to);
            let source_reachable = operation_volume_reachable_for_path(&report, from);
        ...
        _ => "operation-volume-copy-policy\tsource=-\tdestination=-\tsource-class=-\tdestination-class=-\tsource-stable-id=-\tdestination-stable-id=-\tsource-label=-\tdestination-label=-\tsource-volume-root=-\tdestination-volume-root=-\tsource-writable=-\tdestination-writable=-\tsource-read-only=-\tdestination-read-only=-\tsource-reachable=-\tdestination-reachable=-\tbuffer-bytes=0\tfile-cloning=false\tdistinct-volumes=false\thard-links=false\tsparse-files=false\tvolumes=0".to_string(),
    })
}
...
fn operation_volume_writable_for_path(report: &VolumeDiscoveryReport, path: &Path) -> String {
    report
        .volume_for_path(path)
        .map(|volume| volume.writable.to_string())
        .unwrap_or_else(|| "-".to_string())
}

fn operation_volume_read_only_for_path(report: &VolumeDiscoveryReport, path: &Path) -> String {
    report
        .volume_for_path(path)
        .map(|volume| volume.read_only.to_string())
        .unwrap_or_else(|| "-".to_string())
}

fn operation_volume_reachable_for_path(
    report: &VolumeDiscoveryReport,
    path: &Path,
) -> &'static str {
    report
        .volume_for_path(path)
        .map(|volume| format_operation_optional_bool(volume.reachable))
        .unwrap_or("-")
}
...
fn operation_volume_access_tsv(
    ...
    if let Some(volume) = volume_report.volume_for_path(&probe.probe_path) {
        format!(
            "{}\toperation={}\tworker={}\trole={}\tpath={}\tprobe-path={}\tintent=operate\tvolume-id={}\tstable-id={}\tlabel={}\tvolume-root={}\tclass={}\tmount={}\treachable={}\twritable={}\tread-only={}\treason=cached-volume-report",
            prefix,
            operation_kind(operation),
        ...
        format!(
            "{}\toperation={}\tworker={}\trole={}\tpath={}\tprobe-path={}\tintent=operate\tvolume-id=-\tstable-id=-\tlabel=-\tvolume-root=-\tclass=-\tmount=-\treachable=-\twritable=-\tread-only=-\treason=no-containing-volume",
            prefix,
            operation_kind(operation),
            escape_operation_field(&worker),
            probe.requirement.role.as_str(),
