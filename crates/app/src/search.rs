    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                let volume_reports = volume_reports.clone();
                cancellation.check()?;
                runtime.resize_checked(2, "manifest-search:preflight", || cancellation.check())?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                {
                    fail_first_search_retry_probe_attempt(
                ...
                search_phase(&runtime, 1, "manifest-search:read", &cancellation)?;
                let result = read(manifest, &cancellation)?;
                search_phase(&runtime, 2, "manifest-search:complete", &cancellation)?;
                runtime.remember_completion_detail("completed")?;
                Ok(result)
            },
        )?,
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                cancellation.check()?;
                runtime
                    .resize_checked(3, "content-set-search:preflight", || cancellation.check())?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                {
                    fail_first_search_retry_probe_attempt(
                ...
                search_phase(&runtime, 1, "content-set-search:load", &cancellation)?;
            ...
            report.full_hydration
                ...
                search_phase(&runtime, 2, "content-set-search:rank", &cancellation)?;
                search_phase(&runtime, 3, "content-set-search:complete", &cancellation)?;
                runtime.remember_completion_detail("completed")?;
                Ok(ContentIndexSetSearchOutput { diagnostics, hits })
            },
        )?,
        WORKER,
    )
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                cancellation.check()?;
                runtime
                    .resize_checked(3, "content-set-session:preflight", || cancellation.check())?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                {
                    fail_first_search_retry_probe_attempt(
                ...
                search_phase(&runtime, 1, "content-set-session:load", &cancellation)?;
                ...
                search_phase(&runtime, 2, "content-set-session:reuse", &cancellation)?;
                ...
                search_phase(&runtime, 3, "content-set-session:complete", &cancellation)?;
                runtime.remember_completion_detail("completed")?;
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                let volume_reports = volume_reports.clone();
                cancellation.check()?;
                runtime.resize_checked(3, "content-manifest-search:preflight", || {
                    cancellation.check()
                })?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                ...
                search_phase(&runtime, 1, "content-manifest-search:load", &cancellation)?;
                let (live, report) = Indexer::default()
                    .load_live_with_content_manifest_cancellable(
                        records,
                        manifest,
                        &query,
                        &cancellation,
            ...
            report.full_hydration
                ...
                search_phase(&runtime, 2, "content-manifest-search:rank", &cancellation)?;
                search_phase(
                    &runtime,
                    3,
                    "content-manifest-search:complete",
                    &cancellation,
                )?;
                runtime.remember_completion_detail("completed")?;
                Ok(ContentIndexSetSearchOutput { diagnostics, hits })
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                let volume_reports = volume_reports.clone();
                cancellation.check()?;
                runtime.resize_checked(3, "content-manifest-session:preflight", || {
                    cancellation.check()
                })?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                ...
                search_phase(&runtime, 1, "content-manifest-session:load", &cancellation)?;
                ...
                let mut hits = first.search.hits;
                search_phase(&runtime, 2, "content-manifest-session:reuse", &cancellation)?;
                let second = session.search_structured_with_budget_cancellable(
                    &parsed,
                    50,
                    SearchLookupBudget::default(),
                ...
                search_phase(
                    &runtime,
                    3,
                    "content-manifest-session:complete",
                    &cancellation,
                )?;
                runtime.remember_completion_detail("completed")?;
                Ok(ContentIndexSessionOutput { diagnostics, hits })
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                let volume_reports = volume_reports.clone();
                cancellation.check()?;
                runtime.resize_checked(3, "sidecar-budget:preflight", || cancellation.check())?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                {
                    fail_first_search_retry_probe_attempt(
                ...
                search_phase(&runtime, 1, "sidecar-budget:open", &cancellation)?;
                let session = open_sidecar_index_query_session(paths, &cancellation)?;
                cancellation.check()?;
                let parsed = SearchQuery::parse_cancellable(&query, &cancellation)?;
                let report = session.search_structured_with_volume_scope_budget_cancellable(
                ...
                search_phase(&runtime, 2, "sidecar-budget:rank", &cancellation)?;
                let diagnostics =
                    format_sidecar_budget_report(&session, &report, Some(&volume_reports), budget);
                search_phase(&runtime, 3, "sidecar-budget:complete", &cancellation)?;
                runtime.remember_completion_detail("completed")?;
                Ok(SidecarSearchOutput {
                    diagnostics,
                    hits: report.search.hits,
    ...
    scheduled_search_result(
        run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
            ...
            move |cancellation, runtime| {
                ...
                cancellation.check()?;
                runtime
                    .resize_checked(3, "sidecar-volume-scope:preflight", || cancellation.check())?;
                if let (Some(retry_probe), Some(retry_access)) =
                    (retry_probe.as_ref(), retry_access.as_ref())
                {
                    fail_first_search_retry_probe_attempt(
                ...
                search_phase(&runtime, 1, "sidecar-volume-scope:open", &cancellation)?;
                let session = open_sidecar_index_query_session(paths, &cancellation)?;
                cancellation.check()?;
                let parsed = SearchQuery::parse_cancellable(&query, &cancellation)?;
                let report = session.search_structured_with_volume_scope_budget_cancellable(
                ...
                search_phase(&runtime, 2, "sidecar-volume-scope:rank", &cancellation)?;
                let diagnostics = format_sidecar_session_report(
                ...
                search_phase(&runtime, 3, "sidecar-volume-scope:complete", &cancellation)?;
                runtime.remember_completion_detail("completed")?;
                Ok(SidecarSearchOutput {
                    diagnostics,
                    hits: report.search.hits,
...
mod tests {
    ...
    #[test]
    fn search_volume_access_tsv_escapes_control_characters() {
        let root = std::env::temp_dir().join(format!(
            "gfm-search-volume-access-escape-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(
            tsv.starts_with("search\\tprefix\tworker=worker\\nname\\r\trole=records\\trole\\r\t")
        );
