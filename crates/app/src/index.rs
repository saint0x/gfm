...
pub(crate) fn run(command: &str, args: &mut impl Iterator<Item = String>) -> Result<bool> {
    match command {
        ...
        "scan-progress" => {
            ...
            let _write_accesses = access_reports.write_accesses_checked(|| Ok(()))?;
            let volume = access_reports.first_volume();
            let checkpoint = visible_scheduled_result(
                run_scheduled_volume_task_cancellable_with_runtime_and_payload_path(
                    Priority::Visible,
                    JobPayloadKind::Indexing,
                    "index",
                    ...
                    move |cancellation, runtime| {
                        ...
                        let access_reports = access_reports.clone();
                        cancellation.check()?;
                        runtime.resize_checked(3, "scan-progress:preflight", || {
                            cancellation.check()
                        })?;
                        let _root_access =
                            access_reports.root_access_checked(|| cancellation.check())?;
                        index_runtime_phase(&runtime, 1, "scan-progress:writes", &cancellation)?;
                        let _write_accesses =
                            access_reports.write_accesses_checked(|| cancellation.check())?;
                        index_runtime_phase(&runtime, 2, "scan-progress:build", &cancellation)?;
                        let checkpoint = Indexer::default().build_with_progress_cancellable(
                            root,
                            records,
                            progress,
                            &cancellation,
                        )?;
                        index_runtime_phase(&runtime, 3, "scan-progress:complete", &cancellation)?;
                        runtime.remember_completion_detail(format!(
                            "completed:{} scanned:{} inaccessible",
                            checkpoint.scanned_records, checkpoint.inaccessible_records
                        ))?;
                        Ok(checkpoint)
                    },
                )?,
...
fn index_volume_access_tsv(
    ...
    if let Some(volume) = volume_report.volume_for_path(path) {
        format!(
            "{escaped_prefix}\tworker={escaped_worker}{role_field}\tpath={}\tintent={}\tvolume-id={}\tstable-id={}\tclass={}\tmount={}\treachable={}\twritable={}\tread-only={}\tvolume-root={}\tvolume-label={}\tvolume-native-status={}\tvolume-native-reason={}\tvolume-resource-status={}\tvolume-resource-reason={}\tvolume-mount-status={}\tvolume-mount-reason={}\treason=cached-volume-report",
            ...
            format_index_optional_string(volume.native_reason.as_deref()),
            volume
                .resource_status
                .map(|status| status.as_str().to_string())
                .unwrap_or_else(|| "-".to_string()),
            format_index_optional_string(volume.resource_reason.as_deref()),
            volume
                .mount_table_status
                .map(|status| status.as_str().to_string())
                .unwrap_or_else(|| "-".to_string()),
            format_index_optional_string(volume.mount_table_reason.as_deref()),
        )
    } else {
        format!(
            "{escaped_prefix}\tworker={escaped_worker}{role_field}\tpath={}\tintent={}\tvolume-id=-\tstable-id=-\tclass=-\tmount=-\treachable=-\twritable=-\tread-only=-\tvolume-root=-\tvolume-label=-\tvolume-native-status=-\tvolume-native-reason=-\tvolume-resource-status=-\tvolume-resource-reason=-\tvolume-mount-status=-\tvolume-mount-reason=-\treason=no-containing-volume",
            escape_index_tsv_field(&path.to_string_lossy()),
            intent.as_str(),
        )
    }
...
fn format_index_optional_string(value: Option<&str>) -> String {
    value
        .map(escape_index_tsv_field)
...
mod tests {
    ...
    fn index_volume_access_tsv_escapes_control_characters() {
        ...
        assert_eq!(tsv.split('\t').count(), 21, "{tsv}");
