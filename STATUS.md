...
44. Complete automatic producer integration for the durable job payload catalog across remaining specialized extraction, thumbnail, preview, and repair producers beyond the implemented schema-checked typed catalog, round-trip storage, escaped payload metadata, operator catalog report, foreground operation hook, shared volume-scoped producer hook, adaptive scheduled repair/index producer hook, and retriable background content indexing hook.
...
46. Complete automatic producer and GPUI progress-surface integration for persistent job progress restoration across remaining specialized producers beyond the implemented atomic typed snapshot store, planned/running/paused restoration filter, escaped detail metadata, operator progress report, foreground operation hook, shared volume-scoped planned/running/terminal snapshot hook, adaptive scheduled repair/index planned/running/terminal snapshot hook, and retriable background content indexing planned/running/terminal snapshot hook.
...
49. Complete retry/backoff integration across remaining operation, indexing, extraction, thumbnail, preview, and repair producers beyond the implemented jobs-layer failure classification, retryable transient/offline-volume backoff, non-retryable permission/missing-file/corrupt-file/permanent gating, and operator retry-plan report.
