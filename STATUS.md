...
44. Complete automatic producer integration for the durable job payload catalog across remaining specialized indexing, extraction, thumbnail, preview, and repair producers beyond the implemented schema-checked typed catalog, round-trip storage, escaped payload metadata, operator catalog report, foreground operation hook, and shared volume-scoped producer hook.
...
46. Complete automatic producer and GPUI progress-surface integration for persistent job progress restoration across remaining specialized producers beyond the implemented atomic typed snapshot store, planned/running/paused restoration filter, escaped detail metadata, operator progress report, foreground operation hook, and shared volume-scoped producer planned/running/terminal snapshot hook.
