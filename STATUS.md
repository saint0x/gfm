...
27. Finish Finder-visible metadata direct macOS binding for LaunchServices/CoreServices localized kind resolution, FinderInfo alias resolution, bundle display-name rules, sidebar/tag UI propagation, live xattr invalidation, and captured Finder pixel baselines.
...
31. Complete PDF extraction with sandboxed workers, compressed/encrypted PDF coverage, incremental updates, extractor-version invalidation, and corrupt-file quarantine beyond the bounded in-process text-stream extractor.
32. Complete Office extraction beyond bounded OOXML with legacy binary format strategy, protected/encrypted document handling, sandboxed workers, extractor-version invalidation, and corrupt-package quarantine.
33. Complete extraction policy depth beyond current UTF-8 text, HTML, RTF, MIME multipart email, ZIP/TAR/TAR.GZ metadata with PAX/GNU long names, JSON, CSV, and plist paths, including remaining richer archive formats and corrupt-input quarantine.
34. Implement OCR strategy for image-only PDFs and screenshots without blocking primary indexing.
35. Complete hardened extraction-worker isolation with production read-deny Seatbelt feasibility, XPC/App Sandbox entitlement minimization, crash telemetry, and sandbox violation diagnostics beyond the implemented mutation-deny Seatbelt wrapper, timeout-supervised adaptive subprocess worker, and persistent worker-failure quarantine integration.
...
37. Finish Finder-compatible copy preservation for remaining edge behavior beyond the implemented symlink object/timestamp preservation, recursive hard-link topology preservation, source sparse-hole preservation without dense-file sparsification during byte-copy fallback, same-inode no-op and staged top-level regular-file/symlink/directory/package copy replace that preserves the existing destination until final rename with directory/package backup restore on failed install, final-rename-first top-level regular-file/symlink/directory/package move/rename replace with staged fallback where needed and same-inode source-link collapse for regular files, ownership preservation where the host permits it, permissions, access/modified/birth timestamps, post-order recursive directory metadata application, copyable xattrs, macOS ACLs, macOS BSD file flags, Finder-package atomic merge-collision guard, and incomplete fresh directory/package copy rollback on cancellation or failure.
38. Finish native GPUI operation pause/resume progress-surface binding and any remaining recovery UX beyond the implemented operation-engine pause state, paused journal recovery, idempotent paused copy/move/rename destination verification, journaled cancellation checkpoints, byte-copy fallback chunk progress with pause/cancel checkpoints, interrupted-operation recovery replay, explicit capped retry for classified transient failed operations, and Finder-compatible Pause/Resume/Stop progress-sheet control-state contract.
39. Finish conflict UI/state machine binding beyond the implemented operation-engine replace, keep-both, merge-folder, skip, apply-to-all default, and per-target policies with journaled actual destinations or skipped terminal outcomes, including Finder-parity sheet presentation, per-item review table, keyboard/focus behavior, and captured Finder baselines.
40. Finish Finder-native Trash restore integration beyond the implemented GFM metadata-backed restore operation, successful permanent-delete metadata retirement, journaled Empty Trash child deletion with per-entry metadata retirement, and stale Empty Trash metadata reconciliation after interrupted deletion, including direct platform Trash item identity, Put Back parity, collision sheets, native destructive delete sheets, native Empty Trash confirmation/policy binding, and captured Finder baselines.
41. Finish privileged-operation flow for protected paths beyond the implemented operation access gate and macOS security-scope preflight for foreground mutations, including native GPUI permission sheets, security-scoped bookmark acquisition/resolution, privileged helper or authorization strategy where required, Finder-parity Full Disk Access guidance, and captured protected-path baselines.
42. Finish native volume-specific network-volume fallback and slow-volume throttling binding beyond the implemented operation-engine volume-copy policy with local, external, network, and slow fallback checkpoint chunk sizes plus app-level discovered macOS volume-descriptor mapping into operation contexts, including direct DiskArbitration/FileProvider volume classification and captured slow-volume baselines.

## Jobs And Runtime

43. Complete automatic producer integration for the durable job payload catalog across remaining specialized extraction, thumbnail, preview, and repair producers beyond the implemented schema-checked typed catalog, round-trip storage, escaped payload metadata, operator catalog report, foreground operation hook, shared volume-scoped producer hook, adaptive scheduled repair/index producer hook, adaptive extraction worker producer hook, and retriable background content indexing hook.
44. Complete automatic producer integration for dependency-aware fair scheduling across foreground, visible, background, maintenance, and repair queues beyond the implemented classed job API, dependency planner, weighted quotas, deterministic blocked-dependency reporting, and operator fairness report.
45. Complete automatic producer and GPUI progress-surface integration for persistent job progress restoration across remaining specialized producers beyond the implemented atomic typed snapshot store, planned/running/paused restoration filter, escaped detail metadata, operator progress report, foreground operation hook, shared volume-scoped planned/running/terminal snapshot hook, adaptive scheduled repair/index planned/running/terminal snapshot hook, adaptive extraction worker planned/running/terminal snapshot hook, and retriable background content indexing planned/running/terminal snapshot hook.
46. Bind thermal, battery, IO pressure, and user-activity adaptive scheduling into remaining foreground, preview, and thumbnail producers beyond the implemented jobs-layer policy, background content indexing hook, content maintenance hook, sidecar repair hook, persistent-index repair hook, and diagnostics-rebuild hook.
47. Complete automatic producer integration for structured cancellation propagation across remaining operation, indexing, extraction, thumbnail, preview, and repair producers beyond the implemented parent-to-child jobs-layer cancellation tree, child-local branch cancellation, nested worker cancellation checks, search/index cancellation contracts, operation cancellation checkpoints, preview offscreen cancellation, and adaptive extraction subprocess cancellation supervision.
