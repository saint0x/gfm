...
45. Complete extraction policy depth beyond current UTF-8 text, HTML, RTF, email, and ZIP metadata paths, including binary plist parsing, structured JSON/CSV metadata, MIME multipart emails, richer archive formats, per-format invalidation, and corrupt-input quarantine.
