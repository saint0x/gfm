...
38. Finish Finder-compatible copy preservation for ACLs, birthtime, and remaining package/edge behavior beyond the implemented symlink preservation, ownership preservation where the host permits it, permissions, access/modified timestamps, copyable xattrs, and Finder-package atomic merge-collision guard.
...
43. Finish native volume-specific network-volume fallback and slow-volume throttling binding beyond the implemented operation-engine volume-copy policy with local, external, network, and slow fallback checkpoint chunk sizes plus app-level discovered macOS volume-descriptor mapping into operation contexts, including direct DiskArbitration/FileProvider volume classification, live throughput feedback, and captured slow-volume baselines.
