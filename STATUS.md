...
38. Finish Finder-compatible copy preservation for remaining edge behavior beyond the implemented symlink object/timestamp preservation, recursive hard-link topology preservation, source sparse-hole preservation without dense-file sparsification during byte-copy fallback, same-inode no-op and staged top-level regular-file copy replace that preserves the existing destination until final rename, final-rename-first top-level regular-file move/rename replace with staged cross-volume fallback and same-inode source-link collapse, ownership preservation where the host permits it, permissions, access/modified/birth timestamps, post-order recursive directory metadata application, copyable xattrs, macOS ACLs, macOS BSD file flags, Finder-package atomic merge-collision guard, and incomplete fresh directory/package copy rollback on cancellation or failure.
