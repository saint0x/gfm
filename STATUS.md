...
46. Complete automatic producer and GPUI progress-surface integration for persistent job progress restoration across remaining specialized producers beyond the implemented atomic typed snapshot store, planned/running/paused restoration filter, escaped detail metadata, operator progress report, foreground operation hook, shared volume-scoped producer planned/running/terminal snapshot hook, and retriable background content indexing planned/running/terminal snapshot hook.
