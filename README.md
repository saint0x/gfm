...
- bounded legacy Office compound-file text recovery for DOC, XLS, and PPT through normal FAT streams, DIFAT-discovered FAT sectors, and MiniFAT-backed ministreams under the Office extraction budget and fingerprint lane, with deterministic `legacy-office` skips for valid binary packages that contain no recoverable text, plus corrupt/encrypted compound-package quarantine for malformed or protected files
