...
- `crates/preview`: icons, thumbnails, Quick Look preview policy, memory/disk preview cache, request coalescing, pressure-aware visible-window prioritization, cancellation, security decisions, invalidation, and extraction budgets.
...
- per-volume query shards with parallel fanout, bounded per-shard and global top-k merge, and deterministic global ordering
- inline single-shard query and stream dispatch for the common one-local-volume path, with deterministic fanout only when multiple volume shards participate
...
- archive-backed metadata, prefix, substring, and fuzzy lookup with explicit candidate budgets, query-level substring gram and fuzzy key/term caps, direct/fuzzy prefix-candidate dedupe, cancellable sorted batch metadata, prefix, substring, and fuzzy directory scans for uncached query imports, adaptive prefix cutoffs, lookup cache telemetry, truncation telemetry, and mmap-resident sidecars instead of hydrated session heaps
- sidecar-backed `SidecarIndexQuerySession` objects that keep record, column, metadata, prefix, substring, fuzzy, and content mmap archives open across repeated UI query changes while reusing lookup caches, bounded positive and negative content-posting caches, and a bounded hydrated record/column cache, with cancellable persisted-session import/hydration/search execution and poisoned cache state recovered instead of failing machine-wide foreground searches
- one-pass volume bucketing for sidecar posting imports so machine-wide sharded indexes route metadata, prefix, substring, and content candidates to the owning volume, while fuzzy delete-key expansion uses volume-bounded candidate filtering so mounted-volume fanout does not repeat irrelevant global fuzzy terms
...
- content-backed `ContentIndexQuerySession` objects keep mmap record archives and content archive sets/manifests open across repeated content queries, reuse bounded content-posting caches for repeated hits and complete term misses, skip session cache probes, persisted imports, and archive traversal when the query has no remaining content-candidate budget, skip record hydration entirely for complete negative content-term lookups, reuse `FileId`-keyed record caches for repeated candidates, honor cancellation before cache/import/hydration work, recover poisoned cache state instead of failing foreground searches, and return per-query cache-hit/cache-miss telemetry for search-as-you-type integration
...
- bounded legacy Office compound-file text recovery for DOC, XLS, and PPT through normal FAT streams, DIFAT-discovered FAT sectors, and MiniFAT-backed ministreams under the Office extraction budget and fingerprint lane; Workbook/Book streams import bounded BIFF sheet names, LABEL records, formula STRING results, and SST text records, including record-boundary and mid-string SST CONTINUE payloads, before fallback salvage, with deterministic `legacy-office` skips for valid binary packages that contain no recoverable text, plus corrupt/encrypted compound-package quarantine for malformed or protected files, including WordDocument FIB encryption/obfuscation flags and Workbook/Book FILEPASS records
...
- bounded ZIP/TAR/TAR.GZ/TAR.BZ2/TAR.XZ/RAR4/RAR5/7Z/GZIP/BZIP2/XZ archive metadata import, archive-scoped versioning, corrupt-input quarantine, encrypted ZIP/RAR quarantine, RAR5 and 7zAES encoded-header quarantine, and safe `unsupported-archive` skips for valid encoded archive surfaces or split archive volumes that need deeper multi-part decoders
...
The operation engine is journaled. A crash, power loss, unmount, permission denial, cancellation, pause, or network failure leaves an inspectable recovery path instead of mystery state. Foreground mutations are preflighted through the macOS security-scope decision contract before the operation engine touches files, so prompt-required or denied protected paths fail as explicit permission outcomes instead of partial mutations. File copies preserve symlink objects with link timestamps and recursive hard-link topology, try the native macOS `fclonefileat` path first for regular files, fall back to a GFM-owned bounded streaming byte-copy path for unsupported or cross-device copies, preserve source sparse holes in byte-copy fallback without sparsifying dense zero-filled files, select fallback checkpoint chunk sizes from local, external, network, and slow volume classes, and bind discovered macOS volume descriptors into the operation copy policy before execution. Copy tests report whether the host used the APFS clone fast path or byte-copy fallback, emit chunk-level byte progress with live throughput snapshots and full-speed/constrained/slow classifications plus pause/cancel checkpoints during byte-copy fallback, preserve ownership where the host permits it, preserve permissions, access/modified/birth timestamps, copyable xattrs, macOS access control lists, and macOS BSD file flags, classify metadata degradation separately for FinderInfo, resource forks, quarantine flags, generic xattrs, ACLs, ownership, timestamps, flags, symlink times, and hard-link topology, apply recursive directory metadata only after children are materialized, then default regular-file verification to size checks so APFS clone and local-copy fast paths avoid unconditional full-file rereads while retaining explicit streaming byte verification for hostile or diagnostic policies. Top-level regular-file replace no-ops when source and destination are the same inode; otherwise it copies into a hidden sibling stage and verifies that stage before the final replace rename, preserving the existing destination across cancellation or any pre-commit failure. Top-level symlink copy replace also stages the new symlink before final rename so existing destination links survive cancellation and failed pre-commit work. Top-level directory and package copy replace materializes the full replacement tree in a hidden sibling stage, renames the existing destination to a hidden backup only after staging completes, installs the staged tree, and restores the backup if install fails. Top-level regular-file move and rename replace existing file destinations by trying the final rename before any destination deletion, preserve existing destinations across cancellation, use the staged copy fallback for cross-volume moves, and collapse same-inode hard-link moves by removing only the source link. Top-level symlink, directory, and package move/rename replace existing destinations through the same final-rename-first discipline, with staged fallback when direct rename is unavailable. Recursive copy, move, rename, delete, trash, restore, and Empty Trash operations preflight exact item/byte totals, honor cancellation and pause checkpoints during planning and execution, resolve replace, keep-both, merge-folder, skip, apply-to-all, and per-target conflict decisions before journaling the actual terminal outcome, treat Finder packages as atomic items during merge collisions instead of blending bundle internals, remove incomplete fresh directory and package destinations on cancellation or failed copy, record GFM Trash restore metadata, restore trash entries to their metadata-backed original paths, keep the Trash directory while deleting its children during Empty Trash, retire Trash restore metadata only after permanent delete or Empty Trash child deletion succeeds, reconcile stale Empty Trash metadata for children already gone after interrupted deletion, emit completion-backed progress during execution, expose Finder-compatible Pause, Resume, and Stop progress-sheet control states, replay interrupted started operations from the journal with the original operation id, treat skipped operations as durable terminal outcomes, resume paused copy/move/rename operations through idempotent destination verification, and retry classified transient failed operations only under an explicit capped recovery policy.
...
Foreground copy, move, rename, delete, trash, and restore actions enter the operation engine through the same volume-isolated worker admission path while preserving operation journaling and failure records. The binary's command routes are an internal operator/test harness, not the user-facing file manager surface.
...
Quick Look preview and thumbnail generation commands also enter through volume-isolated worker admission before producing preview contracts, and visible preview workers propagate structured cancellation before security classification, FileProvider/iCloud admission, pressure-aware scheduling, cache-policy decisions, or native preview/thumbnail planning. Typed FileProvider state prevents remote placeholders, offline provider items, and in-flight provider operations from entering native Quick Look or thumbnail generation as ordinary local files. Native Quick Look and thumbnail contracts consume raw runtime pressure at admission time, apply the same atomic preview scheduler pressure gate, keep selected-item Quick Look work visible under host pressure, and allow bulk thumbnail prefetch to defer before it competes with interactive navigation, search, or file operations.
...
The scheduler plans foreground, visible, background, maintenance, and repair work through explicit job classes, dependency edges, and weighted fairness quotas. Adaptive scheduling decisions carry those quotas into isolated worker execution, so foreground and visible work keep first-class latency while background indexing, compaction, and repair queues continue making deterministic progress instead of starving behind endless user-visible churn.
...
Background content indexing also consumes explicit runtime pressure signals: saturated I/O or critical thermal pressure defers the durable job before extraction starts, while elevated pressure, low power, or active user input throttles worker admission and shifts fair queue admission toward foreground and visible classes.
...
Versioned parity gate manifests carry capture provenance before their entries:

```tsv
manifest-version	1
profile	macos-build=25A354	appearance=dark	scale=2x	color-profile=display-p3
entry	toolbar	finder.png	gfm.png	3024	1890	governed-masks.tsv	1440	900	active	icon	/tmp/gfm-parity-fixture/icon
```
...
cargo run -p gfm -- release-toolchain
...
Adaptive subprocess extraction workers derive pressure-aware byte budgets before worker-side file reads, run inside a macOS Seatbelt wrapper that denies ambient file reads by default, allows only the worker executable, requested input, scratch state, and required macOS runtime paths, denies network access, denies filesystem mutation except bounded scratch output, are supervised with timeout-bounded process-group termination, bounded post-kill reaping, and retained stdout/stderr failure diagnostics, honor the shared jobs-layer cancellation token, and persist worker timeout, parser-corrupt, parser-encrypted, sandbox-violation, crash classes, and bounded crash-detail evidence into the extraction quarantine store. Set `GFM_EXTRACTION_SANDBOX_READ_MODE=ambient` only for compatibility diagnostics on hosts where the strict profile exposes a missing system-read allowance. Scheduled runtime work uses payload-aware pressure decisions so visible metadata operations stay immediate while visible preview, thumbnail, and extraction work throttle under host IO, thermal, battery, or active-user pressure. Streaming search emits hot name/path metadata hits first and skips the full deep pass when the query has no possible content, fuzzy, proximity, phrase, or structured-expression delta; whole-machine sharded streams run the same deep-delta gate before fanout so scoped volume shards avoid full-pass work when hot results are already final.
...
cargo run -p gfm -- jobs-payload-restore-plan /tmp/gfm-jobs.gfmjobs /tmp/gfm-jobs.gfmprogress
