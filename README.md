...
- sidecar-backed `SidecarIndexQuerySession` objects that keep record, column, metadata, prefix, substring, fuzzy, and content mmap archives open across repeated UI query changes while reusing lookup caches, bounded positive and negative content-posting caches, and a bounded hydrated record/column cache
