...
- Supports APFS clone fast paths through safe native `fclonefileat` bindings with a GFM-owned bounded streaming byte-copy fallback for unsupported or cross-device regular-file copies, source sparse-hole preservation without sparsifying dense zero-filled files during byte-copy fallback, volume-classed local/external/network/slow fallback checkpoint chunk sizes, discovered macOS volume-descriptor binding into operation copy policy, symlink-object and symlink timestamp preservation during top-level and directory copies, recursive hard-link topology preservation, ownership preservation where the host permits it, permission preservation, access/modified/birth timestamp preservation, copyable xattr preservation, macOS ACL preservation, and macOS BSD file flag preservation on copied files and directories; applies recursive directory metadata only after children are materialized; gates foreground mutations through the macOS security-scope decision contract before touching files; emits chunk-level byte progress, live throughput snapshots, full-speed/constrained/slow throughput classes, and pause/cancel checkpoints during byte-copy fallback; defaults copied regular-file verification to size checks so APFS clone and local-copy fast paths do not pay an unconditional full-file reread, while retaining explicit streaming byte comparison for hostile or diagnostic policies; no-ops top-level same-inode regular-file copy replace and otherwise stages top-level regular-file, symlink, directory, and package copy replace in a hidden sibling path before the final replace rename so pre-commit failures preserve the existing destination, with directory/package backup restore if install fails; performs top-level regular-file, symlink, directory, and package move/rename replace through final-rename-first semantics with staged fallback for cross-volume or unavailable direct renames and same-inode source-link collapse for regular files; emits exact preflight item/byte totals and completion-backed progress events for recursive operations including Empty Trash; exposes Finder-compatible Pause, Resume, and Stop progress-sheet control states for foreground operations; journals cancellation distinctly from failures; honors cancellation and pause checkpoints during planning and recursive execution; resolves replace, keep-both, merge-folder, skip, apply-to-all default, and per-target conflict decisions through operation-engine policies before journaling the actual destination or skipped terminal outcome; treats Finder packages as atomic items during merge collisions rather than recursively blending bundle internals; removes incomplete fresh directory and package destinations on cancellation or failed copy while preserving paused-copy resumability; records GFM Trash restore metadata; restores trash entries to their metadata-backed original paths through the same journaled operation engine; keeps the Trash directory while deleting children during Empty Trash; removes Trash restore metadata only after permanent delete or Empty Trash child deletion succeeds; reconciles stale Empty Trash metadata for children already gone after interrupted deletion; replays interrupted started operations from the durable journal with the original operation id; resumes paused copy/move/rename operations through idempotent destination verification before appending completion; retries classified transient failed operations under explicit capped policy; plus atomic renames, trash semantics, conflict dialogs, checksums where necessary, and network-volume fallbacks.
...
4. Content index
   ...
   - Legacy DOC/XLS/PPT files are classified as Office content, fingerprinted with the Office extractor version, parsed through a bounded CFBF directory reader, quarantined when malformed or encrypted, extract recoverable stream text, and skip with an explicit `legacy-office` reason only when no bounded stream importer or fallback salvage can recover text. Workbook/Book streams import bounded BIFF sheet names, LABEL records, formula STRING results, and SST text records, including record-boundary and mid-string SST CONTINUE payloads, before fallback salvage.
   ...
   - HTML, RTF, MIME multipart email, and ZIP/TAR/TAR.GZ/TAR.BZ2/TAR.XZ archive metadata take format-specific extraction paths, including PAX and GNU TAR long-name headers, so markup, transport headers, MIME body parts, control words, and archive entry names become searchable without indexing binary payloads.
   - RAR, 7Z, standalone GZIP, standalone BZIP2, and standalone XZ are classified as archive inputs, fingerprinted with the archive extractor version, and processed through bounded metadata readers where implemented; malformed RAR/7Z inputs quarantine as corrupt archives, encrypted RAR inputs quarantine as encrypted archives, and valid-but-encoded surfaces skip as unsupported until deeper decoders land.
   ...
   - Background content indexing reports deterministic OCR candidate queue records for image-only PDFs and Mac-style screenshot images while keeping primary content extraction bounded and non-blocking; actual OCR execution remains a deferred worker path rather than a foreground indexing dependency.
...
- Query-time prefix, substring, and fuzzy archive lookup:
  ...
  - sidecar-backed query loading cancellably collects bounded metadata, prefix, substring, fuzzy-term, and content candidate IDs first, then hydrates only those mmap record rows and matching columns before ranking;
  ...
  - sharded search fans out the same archive lookup across volume shards, buckets metadata/prefix/substring/content sidecar postings by `VolumeId` in one pass before shard import, filters prefix and substring candidate ids per shard, and expands fuzzy delete-key candidate terms through volume-bounded lookup so mounted-volume fanout does not repeat irrelevant global fuzzy candidates;
  ...
  - sidecar-backed UI search opens record, column, metadata, prefix, substring, fuzzy, and content mmap archives once per `SidecarIndexQuerySession`, retaining lookup caches, bounded positive and negative content-posting caches, and a bounded hydrated record/column cache across repeated query changes while hydrating only the current query's missing candidate records, honoring cancellation before and between persisted-session cache/import/hydration/search phases, and recovering poisoned cache state instead of failing machine-wide foreground searches;
  ...
  - query fuzzy key imports resolve cache misses through one sorted pass over the mmap fuzzy directory, decoding only bounded term heads and avoiding one binary archive lookup per selected delete key;
...
- Use safe native clone operations for same-volume duplicate/copy where supported, and preserve explicit bounded streaming byte-copy fallback behavior for unsupported filesystems and cross-volume copies.
...
- Preserve permissions, access/modified/birth timestamps, copyable xattrs, and macOS ACLs, including Finder tag/comment, quarantine, and resource-fork attributes where the filesystem exposes them.
- Preserve symlink objects and their link targets instead of following links into target file bytes during copy.
- Verify copied regular-file outputs with the configured size or streaming byte comparison policy before reporting operation success.
