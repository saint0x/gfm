...
4. Content index
   ...
   - Legacy DOC/XLS/PPT files are classified as Office content, fingerprinted with the Office extractor version, parsed through a bounded CFBF directory reader, quarantined when malformed or encrypted, extract recoverable stream text, and skip with an explicit `legacy-office` reason only when no bounded stream importer or fallback salvage can recover text. Workbook/Book streams import bounded BIFF sheet names, LABEL records, and SST text records, including record-boundary and mid-string SST CONTINUE payloads, before fallback salvage.
