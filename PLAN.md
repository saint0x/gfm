...
- Query-time prefix, substring, and fuzzy archive lookup:
  ...
  - content-backed UI search opens mmap record archives and content archive sets or manifests once per `ContentIndexQuerySession`, retaining a bounded `FileId`-keyed record cache across repeated content query changes and reporting per-query hydration cache hits and misses;
