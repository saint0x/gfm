...
- Query-time prefix, substring, and fuzzy archive lookup:
  ...
  - sharded search fans out the same archive lookup across volume shards, buckets metadata/prefix/substring/content sidecar postings by `VolumeId` in one pass before shard import, and filters candidate ids per shard without rescanning every posting for every mounted volume;
